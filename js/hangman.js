...
$("#guess").on("input", function() {
    ...
    if (guessEntered !== '' && !isCharactersAlreadyEntered(guessEntered) && isValidGuess(guessEntered)) { // if the player is not deleting a character.
        ...
        if (!isCorrectGuess) {
            ...
            if (incorrectGuess <= MAX_INCORRECT_GUESSES) {
                setHangmanImage(`images/${incorrectGuess}.png`);
            }
        }
        determineGameStatus();
    }
...
function determineGameStatus() {
    if (incorrectGuess > MAX_INCORRECT_GUESSES) {
       ...
       setHangmanImage("images/7.png");
        ...
        setHangmanImage("images/win.png");
    }
}
...
function setGameStatus(text) {
    ...
    $("#hangmanImageContainer").removeClass("col-md-5");
    $("#hangmanImageContainer").addClass("col-md-12");
    $(".header").hide();
}

function setHangmanImage(image) {
    $("#hangmanImage").attr("src", image);
}
